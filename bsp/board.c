...
static void init_trigger_source(PWM_Type *ptr);
static void init_trigger_mux(TRGM_Type *ptr, uint8_t output);
static void init_trigger_target(ADC16_Type *ptr, uint8_t trig_ch);
static hpm_stat_t process_pmt_data(uint32_t *buff, int32_t start_pos, uint32_t len);
static hpm_stat_t process_seq_data(uint32_t *buff, int32_t start_pos, uint32_t len);
...
__IO uint8_t trig_complete_flag = 0;
__IO uint8_t seq_complete_flag = 0;
__IO uint8_t dma_complete_flag = 0;
uint8_t trig_adc_channel[] = {(2U)};
uint8_t seq_adc_channel[] = {(1U), (2U)};

// #define USE_ADC_TRIG 1
#define USE_ADC_SEQ 1
void board_init_adc()
{
    HPM_IOC->PAD[IOC_PAD_PC04].FUNC_CTL = IOC_PAD_FUNC_CTL_ANALOG_MASK; /* ADC_VDC:ADC0.INA0 */
    HPM_IOC->PAD[IOC_PAD_PC05].FUNC_CTL = IOC_PAD_FUNC_CTL_ANALOG_MASK; /* ADC_VAC: ADC0.INA1 */
    HPM_IOC->PAD[IOC_PAD_PC06].FUNC_CTL = IOC_PAD_FUNC_CTL_ANALOG_MASK; /* ADC_NTC: ADC0.INA2 */
    HPM_IOC->PAD[IOC_PAD_PC08].FUNC_CTL = IOC_PAD_FUNC_CTL_ANALOG_MASK; /* ADC_CS: ADC1.INA0 */
    HPM_IOC->PAD[IOC_PAD_PC12].FUNC_CTL = IOC_PAD_FUNC_CTL_ANALOG_MASK; /* ADC_IDC: ADC2.INA0 */
    ...
    cfg.conv_mode = adc16_conv_mode_sequence; // adc16_conv_mode_preemption;
    cfg.adc_clk_div = adc16_clock_divider_4;
    cfg.sel_sync_ahb = true; //(clk_adc_src_ahb0 == clock_get_source(BOARD_APP_ADC12_CLK_NAME)) ? true : false;
    ...
    init_trigger_mux(HPM_TRGM0, TRGM_TRGOCFG_ADCX_PTRGI0A);
    // #endif

    /* Trigger target initialization */
    init_trigger_target(HPM_ADC0, ADC16_CONFIG_TRG0A);
    ...
    // adc16_enable_interrupts(HPM_ADC0, adc16_event_dma_fifo_full);
#endif
#ifdef USE_ADC_SEQ
    ...
    /* initialize an ADC channel */
    // ch_cfg.diff_sel     = adc16_sample_signal_single_ended; //not for hpm6200
    ...
    for (int i = (0U); i < seq_cfg.seq_len; i++)
    {
    ...
    /* Set a DMA config */
    dma_cfg.start_addr = (uint32_t *)core_local_mem_to_sys_address(HPM_CORE0, (uint32_t)seq_buff);
    ...
    /* Trigger mux initialization */
    init_trigger_mux(HPM_TRGM0, TRGM_TRGOCFG_ADC0_STRGI);
    // #endif
    ...
    // adc16_enable_interrupts(HPM_ADC0, adc16_event_seq_single_complete);
...
void isr_adc16(void)
{
    ...
    if (ADC16_INT_STS_DMA_FIFO_FULL_GET(status)) {
        /* Set flag to read memory data */
        dma_complete_flag = 1;
...
SDK_DECLARE_EXT_ISR_M(IRQn_ADC0, isr_adc16);
...
// gptmr_channel_get_default_config(BOARD_GPTMR_TMR_1M, &config);

// gptmr_freq = clock_get_frequency(BOARD_GPTMR_TMR_1M_CLK_NAME);

// Logging(LOG_DEBUG, "gptmr_freq -> %d\n", gptmr_freq);
// config.reload = 100;//gptmr_freq / 1000 * 1000;
// gptmr_channel_config(BOARD_GPTMR_TMR_1M, BOARD_GPTMR_TMR_1M_CHANNEL, &config, false);
// gptmr_start_counter(BOARD_GPTMR_TMR_1M, BOARD_GPTMR_TMR_1M_CHANNEL);

// gptmr_enable_irq(BOARD_GPTMR_TMR_1M, GPTMR_CH_RLD_IRQ_MASK(BOARD_GPTMR_TMR_1M_CHANNEL));
// intc_m_enable_irq_with_priority(BOARD_GPTMR_TMR_1M_IRQ, 1);
...
static hpm_stat_t process_seq_data(uint32_t *buff, int32_t start_pos, uint32_t len)
{
    ...
    {
        Logging(LOG_DEBUG, "Sequence Mode - %s - ", "ADC0");
        Logging(LOG_DEBUG, "Cycle Bit: %02d - ", dma_data[i].cycle_bit);
        Logging(LOG_DEBUG, "Sequence Number:%02d - ", dma_data[i].seq_num);
        Logging(LOG_DEBUG, "ADC Channel: %02d - ", dma_data[i].adc_ch);

        uint32_t vol = dma_data[i].result * 2500 * 2 / 0Xffff;
        Logging(LOG_DEBUG, "Result: %d, %d, %dmV\n", i, dma_data[i].result, vol);
    }
...
void sequence_handler(void)
{
    ...
    if (seq_complete_flag == 1)
    {
        /* Process data */
        process_seq_data(seq_buff, (0U), sizeof(seq_adc_channel));
        board_toggle_red_led();
        /* Clear the flag */
        seq_complete_flag = 0;
    }
...
void dma_handler(void)
{
    ...
    if (dma_complete_flag == 1)
    {
        ...
        // board_toggle_red_led();
        /* Clear the flag */
        Logging(LOG_DEBUG, "DMA finished\n");
        dma_complete_flag = 0;
    }
}
void preemption_handler(void)
{
    ...
    if (trig_complete_flag == 1)
    {
        /* Process data */
        process_pmt_data(pmt_buff, ADC16_CONFIG_TRG0A * sizeof(adc16_pmt_dma_data_t), sizeof(trig_adc_channel));
        board_toggle_red_led();
        /* Clear the flag */
...
// {
//     if (gptmr_check_status(BOARD_GPTMR_TMR_1M, GPTMR_CH_RLD_STAT_MASK(BOARD_GPTMR_TMR_1M_CHANNEL))) {
//         gptmr_clear_status(BOARD_GPTMR_TMR_1M, GPTMR_CH_RLD_STAT_MASK(BOARD_GPTMR_TMR_1M_CHANNEL));
//         board_toggle_red_led();
//     }
// }
// SDK_DECLARE_EXT_ISR_M(BOARD_GPTMR_TMR_1M_IRQ, tick_ms_isr);
...
static hpm_stat_t process_pmt_data(uint32_t *buff, int32_t start_pos, uint32_t len)
{
    ...
    for (int i = start_pos; i < start_pos + len; i++)
    {
        ...
        {
            Logging(LOG_DEBUG, "Preemption Mode - %s - ", "ADC0");
            Logging(LOG_DEBUG, "Trigger Channel: %02d - ", dma_data[i].trig_ch);
            Logging(LOG_DEBUG, "Cycle Bit: %02d - ", dma_data[i].cycle_bit);
            Logging(LOG_DEBUG, "Sequence Number: %02d - ", dma_data[i].seq_num);
            Logging(LOG_DEBUG, "ADC Channel: %02d - ", dma_data[i].adc_ch);
            uint32_t vol = dma_data[i].result * 2500 * 2 / 0Xffff;
            Logging(LOG_DEBUG, "Result: %d, %dmV\n", dma_data[i].result, vol);
            dma_data[i].cycle_bit = 0;
        }
        else
        {
            // Logging(LOG_DEBUG, "invalid data\n");
        }
    }
...
static void init_trigger_mux(TRGM_Type *ptr, uint8_t output)
{
    ...
    trgm_dma_request_config(ptr, 0UL, HPM_TRGM0_DMA_SRC_PWM0_CMP8);
}
...
static void init_trigger_source(PWM_Type *ptr)
{
    ...
    pwm_config_cmp(ptr, (8U), &pwm_cmp_cfg);

    /* Issue a shadow lock */
    pwm_issue_shadow_register_lock_event(HPM_PWM0);

    /* Set comparator channel to generate a trigger signal */
    pwm_output_ch_cfg.cmp_start_index = (8U); /* start channel */
    pwm_output_ch_cfg.cmp_end_index = (8U);   /* end channel */
    pwm_output_ch_cfg.invert_output = false;
    pwm_config_output_channel(ptr, (8U), &pwm_output_ch_cfg);
